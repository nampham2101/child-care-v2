...
export async function CallButton({ className = "" }: { className?: string }) {
  ...
  // The word was a literal here until #53. The number is a fact and is identical in every
  // language; the verb in front of it is copy, and on a German page it has to be German.
  ...
  return (
    ...
    >
      ...
      <span>
        {t("call")} <span className="tabular-nums">{center.phoneDisplay}</span>
      </span>
    </a>
  );
}
