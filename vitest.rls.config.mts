...
/**
 * The row-level security suite, deliberately a separate Vitest run from `test:unit`.
 *
 * `vitest.config.mts` covers pure logic in `lib/` — no network, no credentials, answers in
 * under a second, and runs on every commit without asking anything of the environment. This
 * suite is the opposite on all four counts: it talks to the hosted Supabase project, because
 * row-level security is a database behaviour and mocking it would only test the mock.
 *
 * Keeping them apart means the fast gate stays hermetic. Folding these tests into `lib/`
 * would make `npm run test:unit` fail whenever the network is down or the project is paused,
 * and the failure would look like broken logic rather than an unreachable database.
 */
...
export default defineConfig({
  test: {
    ...
    // The suite reads and writes the same few rows on one shared database. Run in parallel
    ...
    // failure repeated across every table in `test.each`.
    bail: 1,
  },
  resolve: {
